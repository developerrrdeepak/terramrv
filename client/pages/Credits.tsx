...
export default function Credits() {
  ...
  const load = async () => {
    const r = await api<{ balance: number; monthly: Record<string, number> }>("/api/credits");
  ...
  const request = async () => {
    setMessage(null);
    await api("/api/credits/payouts", { method: "POST", body: JSON.stringify({ amount }) });
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        <div className="rounded-lg border bg-card p-6 shadow-sm">
          ...
          <div className="mt-2 text-3xl font-bold">{balance.toFixed(2)} tCO2e</div>
        </div>
      </div>
      ...
      <div className="mt-8 rounded-lg border bg-card p-6 shadow-sm">
        ...
        <div className="flex gap-2">
          <input className="h-10 w-48 rounded-md border bg-background px-3" placeholder="Amount (tCO2e)" value={amount} onChange={(e) => setAmount(e.target.value)} />
          <button className="rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground" onClick={request}>Request</button>
        </div>
