...
export function CropMonitor() {
  ...
  const generateMonitoringData = (): CropMonitoringData[] => {
    ...
    for (let i = 0; i < 90; i += 7) {
      ...
      data.push({
        ...
        temperature:
          25 +
          Math.sin((weeksSinceStart / 26) * 2 * Math.PI) * 8 +
          (Math.random() - 0.5) * 5,
        precipitation: Math.max(
          0,
          20 +
            Math.sin((weeksSinceStart / 13) * 2 * Math.PI) * 15 +
            (Math.random() - 0.5) * 30,
        ),
        soilMoisture: Math.max(
          ...
          Math.min(
            90,
            40 +
              Math.sin((weeksSinceStart / 13) * 2 * Math.PI) * 20 +
              (Math.random() - 0.5) * 15,
          ),
        ),
        growthStage:
          CROP_STAGES[selectedCrop as keyof typeof CROP_STAGES][
            Math.min(5, Math.floor(weeksSinceStart / 2))
          ],
      });
    }
  ...
  const generateYieldPrediction = async (
    crop: string,
    monitoring: CropMonitoringData[],
  ): Promise<YieldPrediction> => {
    // Simulate ML-based yield prediction
    const avgNDVI =
      monitoring.reduce((sum, d) => sum + d.ndvi, 0) / monitoring.length;
    const avgTemp =
      monitoring.reduce((sum, d) => sum + d.temperature, 0) / monitoring.length;
    ...
    const yieldMultiplier =
      (avgNDVI - 0.3) * 2 + Math.min(1, totalPrecip / 500) * 0.5;
    ...
    return {
      ...
      riskAssessment: {
        ...
        disease:
          avgTemp > 28 ? 30 + Math.random() * 20 : 10 + Math.random() * 15,
  ...
  const generateYieldRecommendations = (
    crop: string,
    ndvi: number,
    temp: number,
  ): string[] => {
    ...
    if (ndvi < 0.5) {
      recommendations.push(
        "Consider nitrogen fertilizer application to improve vegetation health",
      );
    }

    if (temp > 30) {
      recommendations.push(
        "Monitor for heat stress and ensure adequate irrigation",
      );
    }

    if (crop === "Rice" && temp < 20) {
      recommendations.push(
        "Cold temperatures may affect growth - monitor closely",
      );
    }

    recommendations.push(
      "Continue regular monitoring for optimal harvest timing",
    );
    ...
    <section className="container mx-auto px-4 py-16">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Real-time crop health analysis and yield forecasting with satellite
          data and ML
        </p>
      </div>
          ...
          <div className="rounded-lg border bg-card p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div
                  className={`text-2xl font-bold ${getHealthColor(cropData.health)}`}
                >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-1 text-xs">
                    {Object.entries(yieldPrediction.factors).map(
                      ([factor, score]) => (
          ...
          {yieldPrediction && (
            <div className="rounded-lg border bg-card p-6 shadow-sm">
              ...
              <div className="space-y-3">
                {Object.entries(yieldPrediction.riskAssessment).map(
                  ([risk, level]) => (
                    <div
                      key={risk}
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-1 rounded text-xs ${getRiskColor(level)}`}
                      >
                        {Math.round(level)}%
                      </span>
                    </div>
